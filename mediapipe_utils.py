...
def decode_bboxes(score_thresh, scores, bboxes, anchors, best_only=False):
    ...
    det_bboxes = det_bboxes * np.tile(det_anchors[:, 2:4], 6) / scale + np.tile(
        det_anchors[:, 0:2], 6
    )
...
def rect_transformation(body, w, h, scale=1.25):
    ...
    body.rect_points = rotated_rect_to_points(
        body.rect_x_center_a,
        body.rect_y_center_a,
        body.rect_w_a,
        body.rect_h_a,
        body.rotation,
    )
...
class LandmarksSmoothingFilter:
    ...
    def apply(self, landmarks, timestamp, object_scale=0):
        ...
        # Initialize filters
        if self.init:
            self.filters = OneEuroFilter(
                self.frequency, self.min_cutoff, self.beta, self.derivate_cutoff
            )
            self.init = False
        ...
        if self.disable_value_scaling:
            ...
            object_scale = (
                object_scale if object_scale else self.get_object_scale(landmarks)
            )
